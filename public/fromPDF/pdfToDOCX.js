let convertButton = document.getElementById('pdf_to_docx_button')
convertButton.disabled = true
let fileInputButton = document.getElementById('fileInput_pdf_to_docx')
...
fileInputButton.addEventListener('change', e => {
    ...
    if (file !== undefined) {
        ...
        convertButton.onclick = async function () {
            ...
            await startConversion(file, 'pdf', 'docx', '', '', emailId)
            loading.style.display = 'none'
            fileInputButton.disabled = false
            convertButton.disabled = false
        }
