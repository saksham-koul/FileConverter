let convertButton = document.getElementById('watermark_pdf_button')
convertButton.disabled = true
let fileInputButton = document.getElementById('fileInput_watermark_pdf')
...
function handleButtons() {
    if (file !== undefined && text.length > 0) {
        ...
        convertButton.onclick = async function () {
            ...
            convertButton.disabled = true
            await startConversion(file, 'pdf', 'watermark', text, '', emailId)
            loading.style.display = 'none'
            fileInputButton.disabled = false
            convertButton.disabled = false
        }
...
document.getElementById('watermark_text').addEventListener('change', e => {
    text = e.target.value
    text = text.trimStart()
    text = text.trimEnd()
    console.log(text)
