let convertButton = document.getElementById('decrypt_pdf_button')
convertButton.disabled = true
let fileInputButton = document.getElementById('fileInput_decrypt_pdf')
...
function handleButtons() {
    if (file !== undefined && pass.length > 0) {
        ...
        convertButton.onclick = async function () {
            ...
            convertButton.disabled = true
            await startConversion(file, 'pdf', 'decrypt', '', pass, emailId)
            loading.style.display = 'none'
            fileInputButton.disabled = false
            convertButton.disabled = false
        }
...
document.getElementById('decrypt_pass').addEventListener('change', e => {
    pass = e.target.value
    console.log(pass)
    handleButtons()
})
