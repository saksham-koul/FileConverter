let convertButton = document.getElementById('mp3_to_m4a_button')
convertButton.disabled = true
let fileInputButton = document.getElementById('fileInput_mp3_to_m4a')
...
fileInputButton.addEventListener('change', e => {
    ...
    if (file !== undefined) {
        ...
        formData.append('mp3_m4a', file, file.name)
        convertButton.onclick = function () {
            ...
            fetch('/convert', options).then(response => {
                ...
                response.json().then(result => {
                    console.log(result)
                    const data = {urls: [result.url], recipient: emailId, from: 'mp3', to: 'm4a'}
                    const options = {
                        method: 'POST',
                        headers: {
