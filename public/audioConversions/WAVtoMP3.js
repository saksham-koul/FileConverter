let convertButton = document.getElementById('wav_to_mp3_button')
convertButton.disabled = true
let fileInputButton = document.getElementById('fileInput_wav_to_mp3')
...
fileInputButton.addEventListener('change', e => {
    ...
    if (file !== undefined) {
        ...
        formData.append('wav_mp3', file, file.name)
        convertButton.onclick = function () {
            ...
            fetch('/convert', options).then(response => {
                ...
                response.json().then(result => {
                    console.log(result)
                    const data = {urls: [result.url], recipient: emailId, from: 'wav', to: 'mp3'}
                    const options = {
                        method: 'POST',
                        headers: {
