let convertButton = document.getElementById('ogg_to_mp3_button')
convertButton.disabled = true
let fileInputButton = document.getElementById('fileInput_ogg_to_mp3')
...
fileInputButton.addEventListener('change', e => {
    ...
    if (file !== undefined) {
        ...
        formData.append('ogg_mp3', file, file.name)
        convertButton.onclick = function () {
            ...
            fetch('/convert', options).then(response => {
                ...
                response.json().then(result => {
                    console.log(result)
                    const data = {urls: [result.url], recipient: emailId, from: 'ogg', to: 'mp3'}
                    const options = {
                        method: 'POST',
                        headers: {
