let convertButton = document.getElementById('pptx_to_pdf_button')
convertButton.disabled = true
let fileInputButton = document.getElementById('fileInput_pptx_to_pdf')
...
fileInputButton.addEventListener('change', e => {
    ...
    if (file !== undefined) {
        ...
        convertButton.onclick = async function () {
            ...
            await startConversion(file, 'pptx', 'pdf', '', '', emailId)
            loading.style.display = 'none'
            fileInputButton.disabled = false
            convertButton.disabled = false
        }
