let convertButton = document.getElementById('mkv_to_mp4_button')
convertButton.disabled = true
let fileInputButton = document.getElementById('fileInput_mkv_to_mp4')
...
fileInputButton.addEventListener('change', e => {
    ...
    if (file !== undefined) {
        ...
        formData.append('mkv_mp4', file, file.name)
        convertButton.onclick = function () {
            ...
            fetch('/convert', options).then(response => {
                ...
                response.json().then(result => {
                    console.log(result)
                    const data = {urls: [result.url], recipient: emailId, from: 'mkv', to: 'mp4'}
                    const options = {
                        method: 'POST',
                        headers: {
                    ...
                    let btn = createButton('Download Video', result.url)
