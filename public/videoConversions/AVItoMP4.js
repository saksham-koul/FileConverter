let convertButton = document.getElementById('avi_to_mp4_button')
convertButton.disabled = true
let fileInputButton = document.getElementById('fileInput_avi_to_mp4')
...
fileInputButton.addEventListener('change', e => {
    ...
    if (file !== undefined) {
        ...
        formData.append('avi_mp4', file, file.name)
        convertButton.onclick = function () {
            ...
            fetch('/convert', options).then(response => {
                ...
                response.json().then(result => {
                    console.log(result)
                    const data = {urls: [result.url], recipient: emailId, from: 'avi', to: 'mp4'}
                    const options = {
                        method: 'POST',
                        headers: {
